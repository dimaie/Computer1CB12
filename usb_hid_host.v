module usb_hid_host (
    ...
    input  rst,	                    // reset (active-high per SAP-3 standard)
    inout  usb_dm, usb_dp,          // USB D- and D+

    output reg [1:0] typ = 0,       // device type. 0: no device, 1: keyboard, 2: mouse, 3: gamepad
...
always @(posedge usbclk) begin : response_recognition
    ...
    if (save) begin
        ...
        if (regs[4] == 3) begin  // bInterfaceClass. 3: HID, other: non-HID
            ...
            else                 // Many modern mice report SubClass 0 (Non-Boot).
                typ <= regs[6] == 1 ? 1 : 2;     // Fallback to treat as keyboard/mouse instead of gamepad
        end else
            typ <= 0;                   
    end
